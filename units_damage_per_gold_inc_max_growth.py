...
for key, value in towns_noup.items():
    for i in range(len(value)):
        ...
        unit_max_growth = value.iloc[i]['max_growth']
        min_damage_per_gold = unit_min_damage * 0.05 * unit_attack * unit_max_growth / unit_cost
        max_damage_per_gold = unit_max_damage * 0.05 * unit_attack * unit_max_growth / unit_cost
...
for unit_name, value, town_name in list_of_damage_per_gold:
    ...
    plt.xlabel('Avarage damage per gold for max growth')
    plt.show()